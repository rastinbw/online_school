...
<div
    ...
    <label style="margin-bottom:15px;margin-top:15px;">{!! $field['label'] !!}</label>
        ...
        <table
            ...
            <thead>
                <tr>
                    ...
                    <th style="font-weight: 600!important;text-align:center;">
                        {{ $column['label'] }}
                    </th>
                    @endforeach
            ...
            <tbody ui-sortable="sortableOptions" ng-model="items" class="table-striped">
                ...
                <tr post-render ng-repeat="item in items" class="array-row" >
                    ...
                    <?php $x = null ?>
                    @foreach ($field['columns'] as $column)
                        <td style="padding: 0px" class="@if(isset($column['size']))  col-md-{{ $column['size'] }} @endif">
                ...
                {{--<tr class="array-row" >--}}
                    {{--<td colspan="{{sizeof($field['columns'])+1}}">جمع کل: <span id="total">0</span> ریال</td>--}}
                {{--</tr>--}}
        ...
        <script>
            ...
            window.angularApp.controller('tableController', function($scope){
                ...
                $scope.removeItem = function(item){
                    ...
                    // if($scope.items[0]['count'] != undefined){
                    //     if(item['cross'] != null){
                    //         $("#total").text(Number($('#total').text()) - item['cross']);
                    //         $("#total").text(addCommas($("#total").text()));
                    //     }
                    // }else {
                    //     if(item['price'] != null) {
                    //         $("#total").text(Number($('#total').text()) - Number(item['price']));
                    //         $("#total").text(addCommas($("#total").text()));
                    //     }
                    // }
                ...
                $scope.$watch('items', function(a, b){
                    ...
                    //         if($scope.items[i]['count'] != null && $scope.items[i]['price'] != null){
                    //             total += $scope.items[i]['count'] * $scope.items[i]['price'];
                    //             $("#total").text(total);
                    //             $("#total").text(addCommas($("#total").text()));
                    //         }
                    //     }
                    // }else {
                    //     for (i=0;i<$scope.items.length;i++) {
                    //         if($scope.items[i]['price'] != null){
                    //             total += Number($scope.items[i]['price']);
                    //             $("#total").text(total);
                    //             $("#total").text(addCommas($("#total").text()));
                    //         }
                    //     }
